<!--
  epic-to-feature-specs — Gemini adapter snippet.

  Paste this section into your project's GEMINI.md (or use it as your GEMINI.md
  if you don't have one). Gemini CLI and Gemini Code Assist load GEMINI.md
  automatically from the project root.

  This snippet keeps Gemini's context cheap: it tells Gemini *when* the workflow
  is relevant and points at the full workflow file. Gemini only loads
...
- **Figma / Pencil design tools** — if any are connected as MCP servers (Gemini CLI extensions or Code Assist tools), use them to pull component trees, screen flows, and design tokens during Step 2.5. Otherwise ask the user for the file URL or screenshots.
...
If `./.epic-to-feature-specs/` doesn't exist in this project, see `adapters/gemini/INSTALL.md` in the upstream repo (`unknown-studio-dev/epic-to-feature-specs`) for the one-command install.
